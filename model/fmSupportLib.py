...
def customizedDemodArctan(I, Q, prev_I_Q=[0.0, 0.0]):
    custom_fm_demod = np.zeros(len(I))
    A = sqrt(pow(I,2)+pow(Q,2))
    # running through each of the I and Q pairs
    
    for k in range(len(I)):
        # following the equation that was given
        # I to the power of 2 plus Q to the power of two
        denominator = pow(I[k],2) + pow(Q[k], 2)
        if (k==0):
            if denominator == 0:
                custom_fm_demod[k] = 0;
            else:
                custom_fm_demod[k] = (I[k] * (Q[k] - prev_I_Q[1]) - Q[k] * (I[k] - prev_I_Q[0]))/denominator
                
        elif (k==1):
            if denominator == 0:
                custom_fm_demod[k] = custom_fm_demod[k-1]
            else:
                custom_fm_demod[k] = (I[k] * (Q[k] - Q[k-1]) - Q[k] * (I[k] - I[k-1]))/denominator
                
                
    prev_I_Q = [I[-1], Q[-1]]
    
    return custom_fm_demod, prev_I_Q
...
## A part of the support library involves the customization of the lowpass filter that would have to be implemented into the stereo blocks.

def customlowpass(Fc, Fs, ntaps):
...
## The convolution function that was produced by us has also been added to the support library

def conv(h, x, state=None):
    if state is None:
        state = [0] * (len(h)-1)
    
    output_sig = np.convolve(x, h)
    
    state = np.roll(state, -len(x))
    state[-len(x):] = x[-(len(h)-1):]
    
    return output_sig[:len(x)], state[:len(h)-1+len(x)]
